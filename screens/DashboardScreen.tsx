...
const DashboardScreen: React.FC<DashboardScreenProps> = ({ onNavigate, userRole }) => {
  ...
  const roleDisplayName =
    userRole === 'HR_ADMIN' ? 'Admin' : userRole === 'MANAGER' ? 'Manager' : 'Employee';
  const welcomeMessage = user?.name
    ? `Selamat Datang, ${user.name}`
    : `Selamat Datang, ${roleDisplayName}`;
    ...
    if (userRole === 'HR_ADMIN') {
      return {
        card1: {
          ...
          sub:
            (stats?.newEmployeesThisMonth ?? 0) > 0 ? `+${stats?.newEmployeesThisMonth} new` : null,
        },
        card2: {
          title: 'Pending Actions',
          value: (stats?.pendingAttendance ?? 0) + (stats?.pendingEwa ?? 0),
        ...
        card3: {
          title: 'Next Payroll',
          value: formatPayrollDate(stats?.nextPayrollDate || null),
          icon: 'payments',
          color: 'text-green-400',
          sub: getCutoffText(stats?.payrollCutoffDate || null),
        },
      };
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="bg-white dark:bg-slate-800 rounded-xl p-6 border border-slate-200 dark:border-slate-700 shadow-sm flex flex-col justify-between h-36 relative overflow-hidden group">
            <div className="absolute right-0 top-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity">
              <span className={`material-symbols-outlined text-6xl ${cards.card1.color}`}>
                {cards.card1.icon}
              </span>
            </div>
          ...
          <div className="bg-white dark:bg-slate-800 rounded-xl p-6 border border-slate-200 dark:border-slate-700 shadow-sm flex flex-col justify-between h-36 relative overflow-hidden group">
            <div className="absolute right-0 top-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity">
              <span className={`material-symbols-outlined text-6xl ${cards.card2.color}`}>
                {cards.card2.icon}
              </span>
            </div>
            <div className="flex flex-col">
          ...
          <div className="bg-white dark:bg-slate-800 rounded-xl p-6 border border-slate-200 dark:border-slate-700 shadow-sm flex flex-col justify-between h-36 relative overflow-hidden group">
            <div className="absolute right-0 top-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity">
              <span className={`material-symbols-outlined text-6xl ${cards.card3.color}`}>
                {cards.card3.icon}
              </span>
            </div>
            <div className="flex flex-col">
          ...
          <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-xl w-full max-w-md mx-4 overflow-hidden">
            ...
            <div className="p-6 space-y-4">
              <div className="flex items-center gap-3 p-3 bg-slate-50 dark:bg-slate-700/50 rounded-xl">
                <div className="size-10 rounded-full bg-blue-100 text-blue-600 flex items-center justify-center text-sm font-bold">
                  {editingActivity.initials}
                </div>
                <div>
