...
const StatusComponent: React.FC<{ status: string }> = ({ status }) => {
  switch (status) {
    ...
    case 'Generating':
      return (
        <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-300">
          <span className="material-symbols-outlined text-[16px] animate-spin">sync</span>Generating
        </span>
      );
    case 'Pending':
      return (
        <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold bg-neutral-100 text-neutral-600 dark:bg-neutral-800 dark:text-neutral-400">
          <span className="material-symbols-outlined text-[16px]">pending</span>Pending
        </span>
      );
    case 'Error':
      return (
        <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold bg-red-100 text-red-600 dark:bg-red-900/30 dark:text-red-400">
...
const PdfPreviewModal: React.FC<{
  ...
  const handlePrint = () => {
    ...
    printWindow.document
      .write(`<!DOCTYPE html><html><head><title>${type === 'payslip' ? 'Payslip' : 'EA Form'}</title>
      ...
      <div className="bg-white dark:bg-[#23220f] rounded-xl shadow-2xl max-w-3xl w-full mx-4 max-h-[90vh] overflow-hidden flex flex-col">
        <div className="flex items-center justify-between px-6 py-4 border-b border-[#e5e5e0] dark:border-[#3e3d25]">
          <h3 className="text-lg font-bold text-[#181811] dark:text-white">
            {type === 'payslip' ? 'Payslip Preview' : 'EA Form Preview'}
          </h3>
          <div className="flex items-center gap-2">
            ...
            <button
              onClick={onClose}
              className="p-2 hover:bg-gray-100 dark:hover:bg-[#3e3d25] rounded-lg transition-colors"
            >
            ...
            {type === 'payslip' && data.type === 'payslip' ? (
              <>
                <div className="header">
                  <div className="company-name">{(data as PayslipPdfData).companyName}</div>
                  <div className="document-title">
                    PAYSLIP - {(data as PayslipPdfData).periodDisplay}
                  </div>
                </div>
                <div className="section">
                  ...
                  <div className="row">
                    ...
                    <span className="value">{data.employeeName}</span>
                  </div>
                  <div className="row">
                    <span className="label">Employee ID</span>
                    <span className="value">{data.employeeCode}</span>
                  </div>
                  <div className="row">
                    <span className="label">Department</span>
                    <span className="value">{(data as PayslipPdfData).department}</span>
                  </div>
                </div>
                <div className="section">
                  ...
                  <div className="row">
                    ...
                    <span className="value">
                      {formatCurrency((data as PayslipPdfData).earnings.basicSalary)}
                    </span>
                  </div>
                  <div className="row">
                    <span className="label">Allowances</span>
                    <span className="value">
                      {formatCurrency((data as PayslipPdfData).earnings.allowances)}
                    </span>
                  </div>
                  <div className="row total-row">
                    <span className="label">Gross Total</span>
                    <span className="value">
                      {formatCurrency((data as PayslipPdfData).earnings.grossTotal)}
                    </span>
                  </div>
                </div>
                <div className="section">
                  ...
                  <div className="row">
                    <span className="label">EPF (11%)</span>
                    <span className="value">
                      {formatCurrency((data as PayslipPdfData).deductions.epfEmployee)}
                    </span>
                  </div>
                  <div className="row">
                    <span className="label">SOCSO</span>
                    <span className="value">
                      {formatCurrency((data as PayslipPdfData).deductions.socsoEmployee)}
                    </span>
                  </div>
                  <div className="row">
                    <span className="label">EIS</span>
                    <span className="value">
                      {formatCurrency((data as PayslipPdfData).deductions.eisEmployee)}
                    </span>
                  </div>
                  <div className="row">
                    <span className="label">PCB (Tax)</span>
                    <span className="value">
                      {formatCurrency((data as PayslipPdfData).deductions.pcb)}
                    </span>
                  </div>
                  <div className="row total-row">
                    <span className="label">Total Deductions</span>
                    <span className="value">
                      {formatCurrency((data as PayslipPdfData).deductions.totalDeductions)}
                    </span>
                  </div>
                </div>
                <div className="net-pay">
                  ...
                  <div style={{ fontSize: '28px', fontWeight: 'bold', color: '#2e7d32' }}>
                    {formatCurrency((data as PayslipPdfData).netPay)}
                  </div>
                </div>
                <div className="footer">
                  <p>
                    Payment Date: {(data as PayslipPdfData).paymentDate} | This is a
              ...
              <>
                <div className="header">
                  <div className="company-name">BORANG EA / EA FORM</div>
                  <div className="document-title">
                    Annual Remuneration Statement - {(data as EaFormData).year}
                  </div>
                </div>
                <div className="section">
                  <div className="section-title">Part A - Employer Details</div>
                  <div className="row">
                    <span className="label">Employer No.</span>
                    <span className="value">{(data as EaFormData).employerNo}</span>
                  </div>
                  <div className="row">
                    <span className="label">Employer Name</span>
                    <span className="value">{(data as EaFormData).employerName}</span>
                  </div>
                </div>
                <div className="section">
                  <div className="section-title">Part B - Employee Details</div>
                  <div className="row">
                    <span className="label">Employee No.</span>
                    <span className="value">{(data as EaFormData).employeeNo}</span>
                  </div>
                  <div className="row">
                    <span className="label">Employee Name</span>
                    <span className="value">{(data as EaFormData).employeeName}</span>
                  </div>
                  <div className="row">
                    <span className="label">IC No.</span>
                    <span className="value">{(data as EaFormData).icNo}</span>
                  </div>
                </div>
                <div className="section">
                  <div className="section-title">Part D - Gross Remuneration</div>
                  <div className="row">
                    <span className="label">Salary/Wages</span>
                    <span className="value">
                      {formatCurrency((data as EaFormData).salaryWages)}
                    </span>
                  </div>
                  <div className="row total-row">
                    <span className="label">Total Gross</span>
                    <span className="value">
                      {formatCurrency((data as EaFormData).totalGrossRemuneration)}
                    </span>
                  </div>
                </div>
                <div className="section">
                  <div className="section-title">Part E - Deductions</div>
                  <div className="row">
                    <span className="label">EPF Contribution</span>
                    <span className="value">
                      {formatCurrency((data as EaFormData).epfContribution)}
                    </span>
                  </div>
                  <div className="row">
                    <span className="label">SOCSO</span>
                    <span className="value">
                      {formatCurrency((data as EaFormData).socsoContribution)}
                    </span>
                  </div>
                </div>
                <div className="section">
                  <div className="section-title">Part F - Tax Deducted</div>
                  <div className="row">
                    <span className="label">PCB Deducted</span>
                    <span className="value">
                      {formatCurrency((data as EaFormData).pcbDeducted)}
                    </span>
                  </div>
                  <div className="row total-row">
                    <span className="label">Total Tax</span>
                    <span className="value">
                      {formatCurrency((data as EaFormData).totalTaxDeducted)}
                    </span>
                  </div>
                </div>
                <div className="footer">
                  <p>Form No: {(data as EaFormData).formNo} | Prepared per Income Tax Act 1967</p>
                </div>
              </>
            )}
          </div>
...
const DocumentCenterScreen: React.FC<DocumentCenterScreenProps> = ({ onNavigate }) => {
  ...
  const [previewModal, setPreviewModal] = useState<{
    ...
    data: PayslipPdfData | EaFormData | null;
    type: 'payslip' | 'ea-form';
  }>({ isOpen: false, data: null, type: 'payslip' });
  ...
  const handleBatchDownload = async () => {
    try {
      ...
      if (response.success) {
        setToast({
          message: `Batch download initiated for ${response.data?.documentCount || selectedEmployeeIds.length} documents`,
          type: 'success',
        });
      }
    } catch (err) {
  ...
  const progressPercent =
    employeeDocs.length > 0 ? Math.round((readyCount / employeeDocs.length) * 100) : 0;
      ...
      <div className="flex-1 p-4 md:p-8">
        <div className="max-w-[1200px] mx-auto flex flex-col gap-6">
          <div className="flex flex-col gap-4">
            <nav className="flex items-center gap-2 text-sm text-[#8c8b5f]">
              <button
                onClick={() => onNavigate('Dashboard')}
                className="hover:text-[#181811] dark:hover:text-white transition-colors"
              >
                Home
              </button>
              <span className="material-symbols-outlined text-[16px]">chevron_right</span>
              <span className="text-[#181811] dark:text-white font-medium">Document Center</span>
            </nav>
            <div className="flex flex-wrap justify-between items-end gap-4">
              <div>
                <h1 className="text-3xl md:text-4xl font-black text-[#181811] dark:text-white tracking-tight mb-2">
                  Document Center
                </h1>
                <p className="text-[#8c8b5f] text-base md:text-lg max-w-2xl">
                  Batch process and generate employee documents for payroll and tax compliance.
                </p>
              </div>
              <div className="flex gap-2">
                <button className="flex items-center gap-2 px-4 py-2 bg-white dark:bg-[#2e2d15] border border-[#e5e5e0] dark:border-[#3e3d25] rounded-full text-sm font-medium text-[#181811] dark:text-white hover:bg-[#f5f5f0] dark:hover:bg-[#3e3d25] transition-colors">
                  <span className="material-symbols-outlined text-[20px]">history</span>History Log
                </button>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6 mt-4">
            <div
              ...
              onClick={() => setSelectedProcess('payslip')}
            >
              <div
                className={`absolute inset-0 rounded-xl blur-sm transition-opacity ${selectedProcess === 'payslip' ? 'bg-primary/20 dark:bg-primary/10 opacity-100' : 'opacity-0 group-hover:opacity-100 group-hover:bg-neutral-200/50 dark:group-hover:bg-neutral-800/50'}`}
              ></div>
              <div
                className={`relative h-full flex flex-col p-6 rounded-xl bg-surface-light dark:bg-surface-dark transition-all ${selectedProcess === 'payslip' ? 'border-2 border-primary' : 'border border-[#e5e5e0] dark:border-[#3e3d25] hover:border-[#8c8b5f] dark:hover:border-neutral-500'}`}
              >
                <div className="flex justify-between items-start mb-4">
                  <div className="p-3 bg-primary/20 rounded-full text-[#181811]">
                    <span className="material-symbols-outlined text-[32px]">receipt_long</span>
                  </div>
                  {selectedProcess === 'payslip' && (
                    <div className="px-3 py-1 bg-primary text-[#181811] text-xs font-bold uppercase rounded-full tracking-wide">
                      Selected
                    </div>
                  )}
                </div>
                <h3 className="text-xl font-bold text-[#181811] dark:text-white mb-2">
                  Payslip Run
                </h3>
                <p className="text-[#8c8b5f] text-sm leading-relaxed mb-6">
                  Generate monthly salary slips for all active employees. Includes standard
                  deductions and bonus calculations.
                </p>
                <div className="mt-auto pt-4 border-t border-[#e5e5e0] dark:border-[#3e3d25] flex items-center text-sm font-medium text-[#181811] dark:text-white">
            ...
            <div
              className="relative group cursor-pointer"
              onClick={() => setSelectedProcess('ea-form')}
            >
              <div
                className={`absolute inset-0 rounded-xl blur-sm transition-opacity ${selectedProcess === 'ea-form' ? 'bg-primary/20 dark:bg-primary/10 opacity-100' : 'opacity-0 group-hover:opacity-100 group-hover:bg-neutral-200/50 dark:group-hover:bg-neutral-800/50'}`}
              ></div>
              <div
                className={`relative h-full flex flex-col p-6 rounded-xl bg-surface-light dark:bg-surface-dark transition-all ${selectedProcess === 'ea-form' ? 'border-2 border-primary' : 'border border-[#e5e5e0] dark:border-[#3e3d25] hover:border-[#8c8b5f] dark:hover:border-neutral-500'}`}
              >
                <div className="flex justify-between items-start mb-4">
                  <div className="p-3 bg-[#f5f5f0] dark:bg-[#3e3d25] rounded-full text-[#8c8b5f] group-hover:text-[#181811] dark:group-hover:text-white transition-colors">
                    <span className="material-symbols-outlined text-[32px]">description</span>
                  </div>
                  {selectedProcess === 'ea-form' && (
                    <div className="px-3 py-1 bg-primary text-[#181811] text-xs font-bold uppercase rounded-full tracking-wide">
                      Selected
                    </div>
                  )}
                ...
                <p className="text-[#8c8b5f] text-sm leading-relaxed mb-6">
                  Process annual tax forms for the fiscal year. Requires finalized annual payroll
                  data.
                </p>
                <div className="mt-auto pt-4 border-t border-[#e5e5e0] dark:border-[#3e3d25] flex items-center text-sm font-medium text-[#8c8b5f]">
                  <span>Scheduled: Dec 31, 2025</span>
                </div>
              </div>
            </div>
          </div>
              ...
              <div className="flex flex-col md:flex-row gap-6 md:items-center justify-between">
                <div className="flex flex-col gap-1">
                  ...
                  <p className="text-sm text-[#8c8b5f]">
                    Batch ID: #PAY-{currentPeriod.replace('-', '')}-001 • {employeeDocs.length}{' '}
                    Employees
                  </p>
                </div>
              ...
              <div className="border rounded-xl border-[#e5e5e0] dark:border-[#3e3d25] overflow-hidden bg-white dark:bg-[#23220f]">
                {loading ? (
                  ...
                  <div className="p-8 text-center">
                    ...
                    <button
                      onClick={fetchEmployeeDocuments}
                      className="mt-4 px-4 py-2 bg-red-100 hover:bg-red-200 rounded-lg text-red-700 text-sm font-medium"
                    >
                      Try Again
                    </button>
                  </div>
                      ...
                      <table className="w-full text-left border-collapse">
                          ...
                          <tr>
                            <th className="p-4 border-b border-[#e5e5e0] dark:border-[#3e3d25] w-12">
                              <input
                                ...
                                checked={
                                  selectedIds.size ===
                                    employeeDocs.filter(d => d.status === 'Ready').length &&
                                  selectedIds.size > 0
                                }
                            ...
                            <th className="p-4 border-b border-[#e5e5e0] dark:border-[#3e3d25]">
                              Employee
                            </th>
                            <th className="p-4 border-b border-[#e5e5e0] dark:border-[#3e3d25]">
                              Department
                            ...
                            <th className="p-4 border-b border-[#e5e5e0] dark:border-[#3e3d25] text-right">
                              Actions
                            </th>
                          </tr>
                        </thead>
                        <tbody className="text-sm">
                          {employeeDocs.map((emp, idx) => (
                            <tr
                              key={emp.id}
                              className="group hover:bg-[#f9f9f7] dark:hover:bg-[#2e2d15]/50 transition-colors border-b border-[#f5f5f0] dark:border-[#3e3d25]"
                            >
                              <td className="p-4">
                                <input
                                  type="checkbox"
                              ...
                              <td className="p-4">
                                <div className="flex items-center gap-3">
                                  {emp.avatarUrl ? (
                                    <div
                                      className="size-9 rounded-full bg-cover bg-center border border-[#e5e5e0] dark:border-[#3e3d25]"
                                      style={{ backgroundImage: `url(${emp.avatarUrl})` }}
                                    ></div>
                                  ) : (
                                  ...
                                  <div>
                                    <p className="font-bold text-[#181811] dark:text-white">
                                      {emp.employeeName}
                                    </p>
                              ...
                              <td className="p-4">
                                <span
                                  className={`inline-flex items-center px-2.5 py-0.5 rounded-md text-xs font-medium ${getDeptColorClass(emp.department)}`}
                                >
                                  {emp.department}
                                </span>
                              </td>
                              <td className="p-4 font-mono text-[#181811] dark:text-white">
                                {formatCurrency(emp.netPay)}
                              </td>
                              <td className="p-4">
                                <StatusComponent status={emp.status} />
                              </td>
                                ...
                                <div className="flex items-center justify-end gap-1">
                                  ...
                                  >
                                    <span className="material-symbols-outlined text-[20px]">
                                      {emp.status === 'Ready' ? 'visibility' : 'visibility_off'}
                                    </span>
                                  </button>
                                  <button
                    ...
                    <div className="flex items-center justify-between p-4 border-t border-[#e5e5e0] dark:border-[#3e3d25] bg-surface-light dark:bg-surface-dark">
                      <p className="text-sm text-[#8c8b5f]">
                        ...
                        <span className="font-medium text-[#181811] dark:text-white">
                          1-{employeeDocs.length}
                        </span>{' '}
                        of{' '}
                        <span className="font-medium text-[#181811] dark:text-white">
                          {pagination.total || employeeDocs.length}
                        </span>{' '}
                        results
                      </p>
                      <div className="flex gap-2">
                        <button
                          className="px-3 py-1 rounded-lg border border-[#e5e5e0] dark:border-[#3e3d25] text-sm text-[#8c8b5f] hover:bg-[#f5f5f0] dark:hover:bg-[#3e3d25] disabled:opacity-50"
                          disabled
                        >
                          Previous
                        </button>
                        <button className="px-3 py-1 rounded-lg border border-[#e5e5e0] dark:border-[#3e3d25] text-sm text-[#181811] dark:text-white hover:bg-[#f5f5f0] dark:hover:bg-[#3e3d25]">
                          Next
                        </button>
                      </div>
                    </div>
