                ...
                <main className="flex-1 flex justify-center py-8 px-4 sm:px-8 lg:px-12 overflow-y-auto">
                    <div className="w-full max-w-[960px] flex flex-col gap-8 pb-20">
                        ...
                        <nav className="flex items-center gap-2 text-sm">
                            <button onClick={() => onNavigate('Dashboard')} className="text-text-sub dark:text-gray-400 hover:text-primary transition-colors font-medium">Home</button>
                        ...
                        <div className="flex flex-col md:flex-row gap-4 items-center">
                            ...
                            <div className="flex gap-2 overflow-x-auto w-full md:w-auto pb-1 md:pb-0">
                                ...
                                >
                                    <span className="material-symbols-outlined text-[18px]">check_circle</span> History
                                </button>
                            </div>
                        </div>
                        ...
                        <div className="flex flex-col gap-6">
                            {loading ? (
                                ...
                                <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-2xl p-6 text-center">
                                    ...
                                    <button onClick={fetchApprovals} className="mt-4 px-4 py-2 bg-red-100 hover:bg-red-200 dark:bg-red-900/30 dark:hover:bg-red-900/50 rounded-lg text-red-700 dark:text-red-300 text-sm font-medium">
                                        Try Again
                                    </button>
                                </div>
                            ) : filteredApprovals.length === 0 ? (
                                <div className="bg-white dark:bg-card-dark rounded-[2rem] p-12 text-center border border-gray-100 dark:border-gray-800">
                                    <span className="material-symbols-outlined text-gray-300 dark:text-gray-600 text-6xl mb-4">task_alt</span>
                                    <h3 className="text-xl font-bold text-text-main dark:text-white mb-2">All caught up!</h3>
                                    <p className="text-text-sub dark:text-gray-400">No pending approvals at the moment.</p>
                                </div>
                            ) : (
                                filteredApprovals.map((approval, idx) => (
                                    <div key={approval.id} className="bg-white dark:bg-card-dark rounded-[2rem] p-6 shadow-sm border border-gray-100 dark:border-gray-800 hover:shadow-md transition-shadow">
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-12 gap-6 mb-8">
                                            <div className="col-span-1 md:col-span-8 bg-background-light dark:bg-background-dark/50 rounded-3xl p-5 border border-gray-100 dark:border-gray-700">
                                                <div className="flex justify-between items-center mb-4 pb-2 border-b border-gray-200 dark:border-gray-700">
                                                    <span className="text-xs font-bold uppercase tracking-wider text-text-sub dark:text-gray-500">Shift Details</span>
                                                    <span className="text-xs font-mono text-text-sub dark:text-gray-500">ID: {approval.id.slice(0, 8).toUpperCase()}</span>
                                                </div>
                                        ...
                                        <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-6 pt-4 border-t border-gray-100 dark:border-gray-800">
                                            <div className="flex flex-col gap-2">
                                                <span className="text-xs font-bold uppercase tracking-wider text-text-sub dark:text-gray-500 pl-1">
                                                    Approval Chain: Step {approval.currentStep} of {approval.totalSteps}
                                                </span>
                                                    ...
                                                    {[1, 2, 3].map((step) => (
                                                        <React.Fragment key={step}>
                                                            ...
                                                            <div className={`flex items-center gap-2 ${step > approval.currentStep ? 'opacity-40 grayscale' : step < approval.currentStep ? 'opacity-50' : ''}`}>
                                                                ...
                                                                <span className={`text-sm font-medium ${step === approval.currentStep ? 'font-bold text-text-main dark:text-white' : 'text-text-main dark:text-gray-400'}`}>
                                                                    {step === 1 ? 'Team Lead' : step === 2 ? 'Manager' : 'Payroll'}
                                                                </span>
                                                            </div>
                                                        </React.Fragment>
                                                    ))}
