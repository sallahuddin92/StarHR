...
const StatusBadge: React.FC<{ status: AttendanceStatus }> = ({ status }) => {
  switch (status) {
    ...
    case 'MISSING_PUNCH':
      return (
        <span className="inline-flex items-center rounded-full bg-red-50 dark:bg-red-900/20 px-2.5 py-0.5 text-xs font-medium text-red-700 dark:text-red-400 ring-1 ring-inset ring-red-600/10">
          MISSING_PUNCH
        </span>
      );
    case 'PENDING_REVIEW':
      return (
        <span className="inline-flex items-center rounded-full bg-gray-100 dark:bg-gray-800 px-2.5 py-0.5 text-xs font-medium text-gray-600 dark:text-gray-300 ring-1 ring-inset ring-gray-500/10">
          PENDING_REVIEW
        </span>
      );
    case 'OT_REJECTED':
      return (
        <span className="inline-flex items-center rounded-full bg-gray-100 dark:bg-gray-800 px-2.5 py-0.5 text-xs font-medium text-gray-500 dark:text-gray-400 ring-1 ring-inset ring-gray-500/10">
          OT_REJECTED
        </span>
      );
  }
...
const AttendanceInterventionScreen: React.FC<AttendanceInterventionScreenProps> = ({
  onNavigate,
}) => {
  ...
  const fetchAttendance = async () => {
    try {
      ...
      const errors = mapped.filter(
        r => r.status === 'MISSING_PUNCH' || r.status === 'OT_REJECTED'
      ).length;
  ...
  const filteredRecords = useMemo(() => {
    return records.filter(r => {
      ...
      // Errors only toggle
      const matchesErrorsOnly =
        !showErrorsOnly || r.status === 'MISSING_PUNCH' || r.status === 'OT_REJECTED';
  ...
  const handleUndo = (id: string) => {
    setRecords(prev =>
      prev.map(r => (r.id === id ? { ...r, verifiedOT: r.initialVerifiedOT } : r))
    );
  };
  ...
  const handleApproveSingle = async (id: string) => {
    ...
    try {
      ...
      setRecords(prev =>
        prev.map(r =>
          r.id === id
            ? { ...r, status: 'NORMAL' as AttendanceStatus, initialVerifiedOT: r.verifiedOT }
            : r
        )
      );
  ...
  const handleResetChanges = () => {
    setRecords(prev =>
      prev.map(r => (selectedIds.has(r.id) ? { ...r, verifiedOT: r.initialVerifiedOT } : r))
    );
        ...
        <div className="flex flex-col gap-6">
          <div className="flex items-center gap-2 text-sm">
            <button
              ...
              className="text-[#8c8b5f] hover:text-primary transition-colors font-medium"
            >
              Home
            </button>
            <span className="text-[#8c8b5f] material-symbols-outlined text-[16px]">
              chevron_right
            </span>
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
            <div className="flex flex-col gap-1">
              <h1 className="text-[#181811] dark:text-white tracking-tight text-3xl font-bold leading-tight">
                Attendance Intervention
              </h1>
              <p className="text-[#8c8b5f] text-base font-normal">
                Audit and correct employee attendance records for payroll processing.
              </p>
            </div>
            <div className="flex gap-4 overflow-x-auto pb-1 md:pb-0">
              <div className="flex min-w-[140px] flex-col gap-1 rounded-xl p-4 border border-[#e6e6db] dark:border-[#3a3a30] bg-white dark:bg-surface-dark shadow-sm">
                <div className="flex items-center justify-between">
                  <p className="text-[#8c8b5f] text-xs font-semibold uppercase tracking-wider">
                    Pending
                  </p>
              ...
              <div className="flex min-w-[140px] flex-col gap-1 rounded-xl p-4 border border-[#e6e6db] dark:border-[#3a3a30] bg-white dark:bg-surface-dark shadow-sm">
                ...
                <p className="text-[#181811] dark:text-white text-2xl font-bold">
                  {stats.approved}
                </p>
              </div>
              <div className="flex min-w-[140px] flex-col gap-1 rounded-xl p-4 border border-[#e6e6db] dark:border-[#3a3a30] bg-white dark:bg-surface-dark shadow-sm">
          ...
          <div className="flex flex-col lg:flex-row gap-4 justify-between items-start lg:items-center">
            <div className="flex flex-wrap items-center gap-3 w-full lg:w-auto">
              <div className="relative w-full md:w-64">
                <span className="absolute left-3 top-1/2 -translate-y-1/2 material-symbols-outlined text-[#8c8b5f]">
                  search
                </span>
                <input
                  ...
                  <tr>
                    <th className="p-4 w-12 text-center">
                      <input
                        ...
                        checked={allSelected}
                        className="rounded border-gray-300 text-primary focus:ring-primary bg-white dark:bg-gray-700 dark:border-gray-600"
                      />
                    </th>
                    ...
                    filteredRecords.map(rec => {
                      ...
                      const rowClasses =
                        rec.status === 'MISSING_PUNCH'
                          ? 'group bg-red-50/30 dark:bg-red-900/5 hover:bg-red-100/50 dark:hover:bg-red-900/10'
                          : 'group hover:bg-[#fafaf7] dark:hover:bg-[#323229]';
                      ...
                      return (
                        <tr key={rec.id} className={`${rowClasses} transition-colors`}>
                          ...
                          <td className="p-4">
                            <div className="flex items-center gap-3">
                              ...
                              <div className="flex flex-col">
                                <span className="font-bold text-[#181811] dark:text-white">
                                  {rec.name}
                                </span>
                          ...
                          <td
                            className={`p-4 font-mono ${rec.status === 'MISSING_PUNCH' ? 'text-red-600 dark:text-red-400 font-medium' : 'text-[#5c5b4f] dark:text-gray-400'}`}
                          >
                            {rec.rawTime}
                          </td>
                          <td className="p-4 text-right font-medium text-[#181811] dark:text-gray-200">
                            {(rec.sysOT ?? 0).toFixed(1)}
                          </td>
                          <td className="p-4 text-right">
                            <div
                              className={`relative inline-block ${isEdited ? 'animate-pulse' : ''}`}
                            >
                              <input
                                type="number"
      ...
      {selectedIds.size > 0 && (
        <div className="bg-white dark:bg-surface-dark border-t border-[#e6e6db] dark:border-[#3a3a30] p-4 px-6 md:px-10 z-30 shadow-[0_-4px_20px_-5px_rgba(0,0,0,0.1)]">
          <div className="max-w-[1400px] mx-auto w-full flex flex-col md:flex-row justify-between items-center gap-4">
            <div className="flex items-center gap-3">
              <div className="flex h-8 w-8 items-center justify-center rounded-full bg-black dark:bg-white text-white dark:text-black font-bold text-sm">
                {selectedIds.size}
              </div>
              ...
              <button
                ...
                className="text-[#8c8b5f] hover:text-[#181811] dark:hover:text-white underline text-sm"
              >
                Unselect all
              </button>
            </div>
              ...
              <div className="flex items-center gap-3 p-3 bg-[#f8f8f5] dark:bg-[#23220f] rounded-xl">
                ...
                <div>
                  ...
                  <p className="text-xs text-[#8c8b5f]">
                    ID: {editingRecord.empId} • {editingRecord.date}
                  </p>
                </div>
              </div>
              <div className="space-y-1">
                <label className="text-sm font-medium text-[#181811] dark:text-white">
                  Clock In
                </label>
