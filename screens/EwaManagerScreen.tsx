...
const EwaManagerScreen: React.FC = () => {
    ...
    return (
        <div className="bg-background-light dark:bg-slate-900 min-h-screen font-body text-slate-800 dark:text-slate-200 flex flex-col">
                    ...
                    <div className="max-w-7xl mx-auto">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-6 mb-8">
                            <div className="bg-white dark:bg-slate-800 rounded-xl p-6 border border-slate-200 dark:border-slate-700 shadow-sm">
                                ...
                                <p className="text-3xl font-bold text-slate-900 dark:text-white mt-2">{formatCurrency(totalDisbursed)}</p>
                            </div>
                            <div className="bg-white dark:bg-slate-800 rounded-xl p-6 border border-slate-200 dark:border-slate-700 shadow-sm">
                                <p className="text-slate-500 dark:text-slate-400 text-sm font-medium uppercase tracking-wider">Available Pool</p>
                                <p className="text-3xl font-bold text-slate-900 dark:text-white mt-2">{formatCurrency(TOTAL_POOL - totalDisbursed)}</p>
                                ...
                                <table className="w-full text-sm text-left">
                                    ...
                                    <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
                                        {isLoading ? (
                                            ...
                                            const safeLimit = req.safeLimit || req.accruedSalary * SAFE_LIMIT_PERCENTAGE;
                                            ...
                                            return (
                                                <tr key={req.id} className={`hover:bg-slate-50 dark:hover:bg-slate-700/30 transition-colors ${isHighRisk ? 'bg-red-50/30 dark:bg-red-900/10' : ''}`}>
                                                    ...
                                                    <td className="px-6 py-4">
                                                      <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-slate-100 text-slate-800 dark:bg-slate-700 dark:text-slate-300">{req.department}</span>
                                                    </td>
            ...
            {/* Toast Notification */}
            {toast && (
                <div className={`fixed top-6 right-6 z-50 flex items-center gap-3 px-6 py-4 rounded-xl shadow-2xl border transition-all ${
                    toast.type === 'success' 
