  ...
  try {
    ...
    if (role === 'HR_ADMIN') {
      ...
      const employeeCountResult = await query(
        'SELECT COUNT(*) as count FROM employee_master WHERE tenant_id = $1 AND is_active = true',
        [tenantId]
      );
      ...
      const mgrResult = await query(`SELECT department FROM employee_master WHERE id = $1`, [
        userId,
      ]);
